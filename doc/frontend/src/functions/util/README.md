[**Smorgasboard**](../../../../README.md)

---

[Smorgasboard](../../../../README.md) / frontend/src/functions/util

# frontend/src/functions/util

This file contains miscellaneous utility functions.

## Remarks

Most of these functions are boolean checks, with a few class/DOM utility
functions in between.

## Author

Radioactive.exe
[GitHub Profile](https://github.com/radioactive-exe)

## Functions

| Function                                                              | Description                                                                     |
| --------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| [areaCollisionWithElement](functions/areaCollisionWithElement.md)     | Checks if an inputted Area collides with a particular HTMLElement.              |
| [collidesWithAnyPanel](functions/collidesWithAnyPanel.md)             | Checks whether an inputted area collides with any panel at all.                 |
| [deleteAfterTransition](functions/deleteAfterTransition.md)           | Deletes/removes an element from the DOM after its transition duration.          |
| [getOptionLabelFromList](functions/getOptionLabelFromList.md)         | Gets the corresponding label for the option with the inputted value.            |
| [isValidOption](functions/isValidOption.md)                           | Checks whether a particular config value is valid/part of the possible options. |
| [removeClassAfterTransition](functions/removeClassAfterTransition.md) | Removes a specified class from an element after its transition duration.        |
| [wouldFit](functions/wouldFit.md)                                     | Checks if the given panels would fit in the potential layout dimensions.        |
