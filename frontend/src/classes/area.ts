/**
 * @file
 * The main class file for the {@link Area} type. Different interfaces, including {@link Coordinate} and {@link Size}, that are utilised in the Area class are also defined here.
 ...
 * A Size object.
 * @remarks
 * This stores the width and height (horizontal and vertical dimensions) of the item they belong to.
 * {@label Size}
 */
interface Size {
    width: number;
    height: number;
    /**
     * Whether this Size is in absolute units (literal pixels) or fractional units (i.e. "cells" in the dashboard grid system).
     * @remarks
     * A value of true means that it is in absolute units, while a value of false (or simply not passing this optional member) implies that the size is fractional.
     * This member is passed when we are passing a Size to functions and constructors, giving us the ability to instantiate and pass Sizes in both fractional and absolute units without having to (a) do a lot of calculations in the function call scope, or (b) take in an extra parameter in the functions.
     */
    isAbsolute?: boolean;
}
 ...
 * @remarks
 * It stores two main things. Firstly, the actual rotation in degrees, and secondly, the offset to any shadow if needed. This is declared to make functions and eliminating code repetition easier.
 */
interface Offset {
    /**
     ...
     * The shadow offset.
     * @remarks
     * This is only needed when there is any 3-dimensional preservation to the rotation, and either the children of the item or the item itself cast a shadow.
     */
    shadow: {
        x: number;
        y: number;
...
class Area {
     ...
     * @remarks
     * This is a static member used when a Panel needs to be initialised from scratch and either (a) does not have a set Area or (b) has one that will be changed right after.
     */
     ...
     * @remarks
     * The constructor creates an Area from a pair of Coordinates and an optional Size input. If no size is passed, default size of \{1, 1\} is used instead.
     * @param coords - The input coordinates (absolute or fractional) to instantiate the Area with. These are passed to and dealt with inside {@link setCoordinates} .
     * @param size - The input size (absolute or fractional) to instantiate the Area with. These are passed to and dealt with inside {@link setSize} .
     * @example
     * ```ts
     * const a = new Area({x: 0, y: 0});
     * ```
     * Instantiates a new Area positioned at the origin (top left of the dashboard/container) and with a default size of `{width: 1, height: 1}`.
     */
    public constructor(
        coords: Coordinate,
     ...
     * @remarks
     * The position is returned as as an object of type {@link Coordinate}, expressed in fractional units ({@link pos}).
     ...
     * @remarks
     * This setter deals with both absolute and fractional sizes. If an absolute set of coordinates is passed, it is converted to fractional units to be stored as such. As such, the {@link pos} member will always store the coordinates in units.
     * @param coords - The input coordinates to set the Area's {@link pos} to.
     * @example
     * ```ts
     * areaOne.setCoordinates({x: 200, y: 250, isAbsolute: true});
     * ```
     * Sets `areaOne`'s coordinates with literal pixel values 200 and 250. In other words, `areaOne` will be positioned 200 pixels horizontally and 250 pixels vertically inside the dashboard/container.
     */
    ...
    /**
     * Returns the fractional Size of the Area, as an object of type {@link Size}.
     ...
     * @remarks
     * This setter deals with both absolute and fractional sizes. If an absolute Size is passed, it is converted to fractional units to be stored as such. As such, the {@link size} member will always store a size in units.
